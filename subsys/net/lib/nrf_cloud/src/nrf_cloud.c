...
int nrf_cloud_shadow_transform_request(char const *const transform, const size_t max_response_len)
{
	...
	ret = nrf_cloud_obj_str_add(&req_obj, NRF_CLOUD_TRANSFORM_REQ_TF_KEY, transform, false);
	if (ret) {
		ret = -ENOMEM;
		goto cleanup;
	}
...
start:
	...
	while (true) {
		...
		if (ret < 0) {
			LOG_ERR("zsock_poll() returned an error: %d; revents: 0x%x",
				ret, fds[0].revents);
		...
		if ((fds[0].revents & ZSOCK_POLLNVAL) == ZSOCK_POLLNVAL) {
			LOG_DBG("Socket error: ZSOCK_POLLNVAL");
		...
		if ((fds[0].revents & ZSOCK_POLLHUP) == ZSOCK_POLLHUP) {
			LOG_DBG("Socket error: ZSOCK_POLLHUP");
		...
		if ((fds[0].revents & ZSOCK_POLLERR) == ZSOCK_POLLERR) {
			LOG_DBG("Socket error: ZSOCK_POLLERR");
